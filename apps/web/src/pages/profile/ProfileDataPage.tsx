import { useNavigate } from "react-router-dom";
import { Download, Upload, ChevronRight } from "lucide-react";
import { PageWrapper } from "../../components/layout/PageWrapper";
import { DetailHeader } from "../../components/DetailHeader";
import { useI18n } from "../../i18n/useI18n";

export function ProfileDataPage() {
  const { t } = useI18n();
  const navigate = useNavigate();

  const menuItems = [
    { icon: Upload, label: t("screens.profile.importData"), to: "/profile/import" },
    { icon: Download, label: t("screens.profile.exportData"), to: "/profile/export" },
  ];

  return (
    <PageWrapper maxWidth="max-w-2xl">
      <DetailHeader title={t("screens.profile.data")} />

      <div className="space-y-1">
        {menuItems.map(({ icon: Icon, label, to }) => (
          <button
            key={to}
            onClick={() => navigate(to)}
            className="w-full flex items-center gap-3 px-4 py-3.5 bg-surface rounded-lg hover:bg-surface-light transition-colors text-left"
          >
            <Icon size={20} className="text-text-muted shrink-0" />
            <span className="text-text text-sm font-medium flex-1">{label}</span>
            <ChevronRight size={18} className="text-text-muted" />
          </button>
        ))}
      </div>
    </PageWrapper>
  );
}
