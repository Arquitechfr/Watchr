---
name: watchr-mobile-desktop-sync
description: Génère un prompt Devin structuré pour synchroniser une page/feature Watchr entre le client mobile (React Native/Expo) et le client desktop (Vite + React), à parité UI et logique métier. Utiliser ce skill dès que ArquiTech mentionne une désynchronisation, un décalage, ou un besoin d'aligner une page entre mobile et desktop sur Watchr — même formulé de façon informelle ("la page X mobile est pas à jour vs desktop", "il faut réaligner les deux clients", "sync mobile/web sur Watchr"). Ne pas utiliser pour Pato, Kloopa ou Falar sauf demande explicite d'adapter le template à ces projets.
---
